...
class TestMakeJsonSafe:
    ...
    def test_bytes_converted_to_hex(self):
        from heroes_platform.telegram_mcp.supabase_writer import _make_json_safe
...
class TestEventHandlers:
    ...
    def test_get_chat_type_channel(self):
        from heroes_platform.telegram_mcp.event_handlers import _get_chat_type
        chat = MagicMock()
        chat.broadcast = True
...
class TestDeploymentFiles:
    ...
    def test_dockerfile_laba_exists(self):
        path = os.path.join(self._project_root(), "heroes_platform", "telegram_mcp", "Dockerfile.laba")
        assert os.path.exists(path)

    def test_docker_compose_laba_exists(self):
        path = os.path.join(self._project_root(), "heroes_platform", "telegram_mcp", "docker-compose.laba.yml")
        assert os.path.exists(path)

    def test_requirements_laba_exists(self):
        path = os.path.join(self._project_root(), "heroes_platform", "telegram_mcp", "requirements-laba.txt")
        assert os.path.exists(path)

    def test_deploy_script_exists(self):
        path = os.path.join(self._project_root(), "heroes_platform", "telegram_mcp", "scripts", "deploy-to-laba.sh")
        assert os.path.exists(path)
    ...
    def test_dockerfile_copies_init_files(self):
        ...
        assert "heroes_platform/telegram_mcp/__init__.py" in content

    def test_healthcheck_matches_cmd(self):
        """ARCHITECTURE: Healthcheck must match the actual process name from CMD."""
        dockerfile = os.path.join(self._project_root(), "heroes_platform", "telegram_mcp", "Dockerfile.laba")
        compose = os.path.join(self._project_root(), "heroes_platform", "telegram_mcp", "docker-compose.laba.yml")
        with open(dockerfile) as f:
            df_content = f.read()
        with open(compose) as f:
            dc_content = f.read()
        # CMD runs heroes_platform.telegram_mcp.main
        assert "heroes_platform.telegram_mcp.main" in df_content
        # Healthcheck should search for the same process
        assert "heroes_platform.telegram_mcp.main" in df_content
        assert "heroes_platform.telegram_mcp.main" in dc_content
